...
class RGBDVideoProcessor(ProcessorMixin):
    ...
    def __init__(self, vision_tower_name, num_frames=24, tokenizer=None, **kwargs): # NOTE: this num_frames parameter specifies how many frames we are allowed to use from the video. We can play around with this to get better results.
        ...
        with open(os.path.join(os.path.join(os.getcwd()[:os.getcwd().index("LLaVA-3D")]), 'LLaVA-3D/playground/data/annotations/embodiedscan_infos.json'), 'r') as file: # NOTE: for generalization, this is a symlink to a file set from a script.
